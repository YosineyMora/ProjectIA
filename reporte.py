...
class reportePDF(object):
    ...
    def _encabezadoPiePagina(canvas, archivoPDF):
        ...
        # Encabezado
        encabezadoNombre = Paragraph("IA - Sistema de detección Facial", estilos["Normal"])
...
def generarReporte(nameP):
    ruta = "Results\\Individual\\"
    rutaDeteccion = ruta+"\\"+nameP+"\\"+nameP
    rutaDesconocido = ruta+"\\"+nameP+"\\"+"Desconocido"
    ...
    datos = [{"NOMBRE": nameP, "ACIERTOS": cantDetec, "FALLOS": cantDesc, "TOTAL": contenidoTotal,"PACIERTOS":str(pA)+"%","PFALLOS":str(pD)+"%"}]

    # conexionDB.close()

    titulo = "Informe Individuo"
